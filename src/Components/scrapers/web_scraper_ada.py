...
def scrape_article(url, sitename:str,path:str):
    ...
    """
    try:
        s = get_parsed_html(url).find_all('div', {'class': 'row bg-white cat-b-row mt-3'})
        ...
        for idx, details in tqdm (enumerate(s,start=1)):
            try:
                ...
                article = get_parsed_html(content_url)
                
                title = article.find('h1',  class_ = 'single-head').text    
...
if __name__ == "__main__":
    ...
    SOURCE_URL_LOCAL  = config['SOURCE_URL_ADA_LOCAL']
    SOURCE_URL_SPORTS = config['SOURCE_URL_ADA_SPORTS']
    SOURCE_URL_INT =config['SOURCE_URL_ADA_INT']
    SOURCE_URL_BUS =config['SOURCE_URL_ADA_BUSINESS']

    SITE_NAME = 'ada'
    TOTAL_PAGES = 200   # TOTAL_PAGES has to be one more than the pages needed to be scraped

    SOURCE_URL = SOURCE_URL_INT
    category = 'Sports_news'  # Add category as needed
    
    for page in tqdm(range(1 ,TOTAL_PAGES), desc=f"Scraping {SITE_NAME}", unit="page"):
        url = f"{SOURCE_URL}/{page*30}"
