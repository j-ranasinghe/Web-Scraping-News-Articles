...
def scrape_article(url, sitename:str,path:str):
    ...
    """
    try:
        s = get_parsed_html(url).find_all('div', {'class': 'tdb_module_loop td_module_wrap td-animation-stack td-cpt-post'})
        ...
        for idx, details in tqdm (enumerate(s,start=1)):
            try:
                ...
                content = article.find('div', class_ = 'td-post-content tagdiv-type')
                context = content.find('p').text
...
if __name__ == "__main__":
    ...
    SOURCE_URL_SPORTS = config['SOURCE_URL_DIVAINA_SPORTS']
    SOURCE_URL_INT =config['SOURCE_URL_DIVAINA_INT']
    SOURCE_URL_BUS =config['SOURCE_URL_DIVAINA_BUSINESS']

    SITE_NAME = 'divaina'
    TOTAL_PAGES = 202   #stopped at 61

    SOURCE_URL = SOURCE_URL_INT
    category = 'International-news'
    
    for page in tqdm(range(62 ,TOTAL_PAGES), desc=f"Scraping {SITE_NAME}", unit="page"):
